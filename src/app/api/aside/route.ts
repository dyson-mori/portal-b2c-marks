...
export async function GET() {
  const aside = await prisma.aside.findMany({
    include: {
      categories: true,
    }
  });
...
export async function POST(request: NextRequest) {
  const body = await request.json() as Aside;

  const category = await prisma.aside.create({
    data: {
      title: body.title,
    },
  });
...
export async function PUT(request: NextRequest) {
  const body = await request.json() as { category: Category, aside: Aside };

  const category = await prisma.aside.update({
    where: {
      id: body.aside.id
    },
    data: {
